...
Status: **local and hosted OpenAPI validation PASS; private publication activation pending**.
...
## Hosted verification
Implementation commit f1e0fc10c216c3547a1a1b30aeaf1cad9f1e9a3e passed both push and PR OpenAPI workflows. [PR run 35329872016](https://github.com/joewschang/UCell/actions/runs/35329872016) validated merge commit 314ba535b059f38192430ed91a86b79d4f84be7e with 24 publisher tests, 73 API suites / 734 tests and 154 DB assertions. Downloaded artifact 10540587335 matches the local generated bytes exactly. Full receipt: [hosted-verification.json](governance/swaggerhub/hosted-verification.json). The publish job was correctly skipped; no published SHA/version is claimed.

The first legacy RC run failed because it invoked db-golden-e2e directly without fixtures. CI now uses the existing db:golden and test:api:isolated runners, retaining non-API tests and the TODO gate. The isolated golden runner passed locally; the final PR CI result remains authoritative.
