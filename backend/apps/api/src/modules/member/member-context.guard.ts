...
export class MemberContextGuard implements CanActivate {
 ...
 async canActivate(context:ExecutionContext){
  ...
  if(['/api/v1/member/me','/api/v1/member/qualifications','/api/v1/member/products','/api/v1/member/profile','/api/v1/member/delivery-profile','/api/v1/member/logout','/api/v1/member/organization/tree'].includes(path)||path.startsWith('/api/v1/member/contracts/')||(path==='/api/v1/member/orders'&&request.method==='POST'&&request.body?.packageVersionId))return true;
