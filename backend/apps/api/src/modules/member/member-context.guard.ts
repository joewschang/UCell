...
export class MemberContextGuard implements CanActivate {
 ...
 async canActivate(context:ExecutionContext){
  ...
  if(['/api/v1/member/me','/api/v1/member/qualifications','/api/v1/member/products','/api/v1/member/profile','/api/v1/member/notifications','/api/v1/member/logout'].includes(path))return true;
  const id=request.method==='POST'?request.body?.qualificationId:request.query?.qualificationId;
