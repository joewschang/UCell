...
export class MemberContractService {
 ...
 async formalRequired(personId:string,at=new Date()){const rows=await this.db.contractDocumentVersion.findMany({where:{required:true,audience:{in:['FORMAL_MEMBER','ALL_MEMBERS']},effectiveFrom:{lte:at},OR:[{effectiveTo:null},{effectiveTo:{gt:at}}]},include:{consentEvidence:{where:{personId},take:1}},orderBy:[{contractType:'asc'},{effectiveFrom:'desc'}]});return rows.map(row=>({id:row.contractDocumentVersionId,type:row.contractType,version:row.versionCode,title:row.title,content:row.contentText,contentHash:row.contentHash,required:row.required,effectiveFrom:row.effectiveFrom.toISOString(),effectiveTo:row.effectiveTo?.toISOString()??null,acceptedAt:row.consentEvidence[0]?.acceptedAt.toISOString()??null}));}
 async consent(personId:string,versionId:string,input:{accepted:boolean;channel:'MEMBER_WEB'|'LIFF'},key:string,requestId:string){
  ...
  try{const result=await this.idempotency.execute(`member:contract:consent:${personId}`,key,{versionId,...input},async tx=>{
   ...
   if(!['NETWORK_MEMBER','FORMAL_MEMBER','ALL_MEMBERS'].includes(contract.audience))throw new NotFoundException({code:'CONTRACT_VERSION_NOT_AVAILABLE'});
