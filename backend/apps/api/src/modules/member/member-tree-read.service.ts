...
export class MemberTreeReadService {
 ...
 async children(personId:string,input:{ballNo:string;parentBallNo?:string;after?:string;snapshotToken?:string;timezone:string;asOf:string;periodStart:string;periodEnd:string;knowledgeCutoff:string}){
  const {ballNo,parentBallNo,after,snapshotToken,...timeInput}=input;
  const time=this.time(timeInput);
  const selected=await this.db.qualification.findUnique({where:{ballNo},include:{binaryTreeMembership:true}});
  ...
  const parent=parentBallNo
   ?await this.db.qualification.findUnique({where:{ballNo:parentBallNo},include:{binaryTreeMembership:true}})
   :selected;
  ...
  const result=await readTreeNodeSnapshot(this.db,{personId,sessionId:'member-tree',provider:'MEMBER',subject:personId,role:'MEMBER'},selected.binaryTreeMembership.binaryTreeId,time,after,snapshotToken,parent.qualificationId);
