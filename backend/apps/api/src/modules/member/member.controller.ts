...
export class MemberCreateOrderDto {
 ...
 @ApiPropertyOptional({pattern:'^[A-Z][A-Z0-9_-]{1,39}$',description:'Untrusted Sponsor Ball or governed Company Alias candidate. Accepted only for a qualification package; server resolves it again and records evidence. It never creates a relationship for retail orders.'}) @IsOptional() @Matches(/^[A-Z][A-Z0-9_-]{1,39}$/) sponsorCode?:string;
 @ApiPropertyOptional({pattern:'^[A-Z][A-Z0-9_-]{0,39}(?:X\\d{6,}|\\d{6,})$',description:'Optional WEB_MEMBER retail referral Ball candidate. Validation is non-binding; the server revalidates it only when the first attributed retail order is committed.'}) @IsOptional() @Matches(/^[A-Z][A-Z0-9_-]{0,39}(?:X\d{6,}|\d{6,})$/) retailReferralCode?:string;
}
...
export class MemberController {
 ...
 @Get('retail-orders') @ApiOperation({operationId:'memberWebRetailOrders',description:'Returns only the authenticated WEB_MEMBER retail-order history, using business order numbers and excluding Qualification, recipient, referrer, payout, and UUID fields.'}) retailOrders(@Req() req:any){return {data:this.orderService.listWebRetailMember(req.user.personId)};}
 ...
 async createOrder(@Req() req:any,@Body() body:MemberCreateOrderDto,@Headers('idempotency-key') key:string){const result=await this.orderService.createMember(body,key,req.requestId,req.user.personId),order=result.value;return {qualificationId:order.qualificationId,id:order.orderId,orderNo:order.orderNo.toString(),status:order.status,total:order.netAmount.toString(),paymentStatus:'PENDING',shipmentStatus:'FULFILLMENT_PENDING',createdAt:new Date(order.createdAt).toISOString(),replayed:result.replayed,lines:order.lines.map(line=>({productId:line.productId,name:line.productNameSnapshot,quantity:line.quantity.toString(),unitPrice:line.unitPrice.toString(),amount:line.lineAmount.toString(),pv:null}))};}
