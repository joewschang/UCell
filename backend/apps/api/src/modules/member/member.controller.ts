...
export class MemberCreateOrderDto {
 ...
 @ApiPropertyOptional({pattern:'^[A-Z][A-Z0-9_-]{0,39}(?:X\\d{6,}|\\d{6,})$',description:'Untrusted Sponsor Ball candidate. Accepted only for a qualification package; server resolves it again and records evidence. It never creates a relationship for retail orders.'}) @IsOptional() @Matches(/^[A-Z][A-Z0-9_-]{0,39}(?:X\d{6,}|\d{6,})$/) sponsorCode?:string;
