...
class ResolvePaperDuplicateDto extends NewPaperPersonDto {
 @IsUUID() reviewId!:string;
 @IsString() @Matches(/^(REUSE_EXISTING_PERSON|APPROVE_NEW_PERSON)$/) decision!:string;
 @IsOptional() @IsString() @MaxLength(80) existingMemberNo?:string;
 @IsString() @Matches(/^\S(?:.*\S)?$/) @MaxLength(128) reasonCode!:string;
}
...
export class PaperIntakeController {
 ...
 createNewPerson(@Body() dto:NewPaperPersonDto,@Headers('idempotency-key') key:string,@Req() req:any){return this.service.createNewPerson({...dto,key,actorId:req.user?.personId,requestId:req.requestId}).then(result=>({data:result.value,meta:{replayed:result.replayed}}));}
 @Post('duplicate-reviews/resolve') @Roles('SUPER_ADMIN','MEMBERSHIP_OPS') @UseGuards(IdempotencyGuard) @ApiHeader({name:'Idempotency-Key',required:true}) @ApiOperation({operationId:'adminResolvePaperDuplicateReview',description:'Authorized manual duplicate review only. Reuses an explicitly selected Person or approves a new Person; it never merges Persons and revalidates the submitted identity fingerprint.'})
 resolveDuplicate(@Body() dto:ResolvePaperDuplicateDto,@Headers('idempotency-key') key:string,@Req() req:any){return this.service.resolveDuplicateReview({...dto,key,actorId:req.user?.personId,requestId:req.requestId}).then(result=>({data:result.value,meta:{replayed:result.replayed}}));}
