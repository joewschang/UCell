...
export class SystemAssignmentService {
 ...
 async create(input:{personId:string;planLevelCode:'STARTER'|'ELITE'|'LEADER';policyVersion:string;effectiveAt?:string},key:string,requestId:string,actorId?:string,now=new Date()){
  try{return await this.idempotency.execute(`admin:qualification:system-assignment:${actorId??'system'}`,key,input,async tx=>{
   ...
   await tx.sponsorRelationship.create({data:{sponsorQualificationId:selected.entry.qualificationId,childQualificationId:qualification.qualificationId,sponsorSequenceNo,effectiveFrom:effectiveAt}});await this.organization.createBinaryPlacement(tx,{parentQualificationId:slot.parentQualificationId,childQualificationId:qualification.qualificationId,side:slot.side,effectiveFrom:effectiveAt},{sourceType:'SYSTEM_ASSIGNMENT',actorId,actorType:actorId?'ADMIN':'SYSTEM',reason:'Explicit versioned system assignment policy '+policy.version});
