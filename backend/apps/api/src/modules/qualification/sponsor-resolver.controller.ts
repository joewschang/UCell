...
class SponsorCodeQuery { @IsString() @Matches(/^[A-Z][A-Z0-9_-]{0,39}(?:X\d{6,}|\d{6,})$/) code!:string; }
...
export class SponsorResolverController {
 ...
 @Get('candidate') @ApiQuery({name:'code',description:'Candidate Sponsor Ball number only; memberNo and UUID are rejected.'})
 ...
 candidate(@Query() query:SponsorCodeQuery){return this.resolver.resolve({code:query.code}).then(data=>({data}));}
}