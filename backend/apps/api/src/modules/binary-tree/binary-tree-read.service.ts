...
export class BinaryTreeReadService {
 ...
 async list(p:TreePrincipal,raw:unknown,after?:string){
  ...
  const data=await this.db.$transaction(async tx=>{
   ...
   for(const tree of trees.slice(0,50)){
    ...
    if(event)items.push({binaryTreeId:tree.binaryTreeId,treeCode:tree.treeCode,treeName:event.treeName,status:event.status,statusVersion:event.topologyVersion,economicActivation:'PENDING_MAPPING'});
   }
 ...
 async nodes(p:TreePrincipal,id:string,raw:unknown,after?:string){
  const time=this.time(raw),at=new Date(time.asOf),known=new Date(time.knowledgeCutoff);await this.commands.authorize(p);
  const result=await this.db.$transaction(async tx=>{
   const visible=await tx.binaryTreeStatusEvent.findFirst({where:{binaryTreeId:id,effectiveAt:{lte:at},recordedAt:{lte:known}}});
   if(!visible)return {status:'UNAVAILABLE',time,items:[],nextCursor:null,total:null};
   const root=await tx.companySponsorDesignation.findUnique({where:{binaryTreeId:id}});
   if(!root||root.effectiveAt>at||root.recordedAt>known)return {status:'UNAVAILABLE',time,items:[],nextCursor:null,total:null};
   const members=await tx.binaryTreeMembership.findMany({where:{binaryTreeId:id,effectiveFrom:{lte:at},recordedAt:{lte:known},...(after?{qualificationId:{gt:after}}:{})},orderBy:{qualificationId:'asc'},take:101});
   const total=await tx.binaryTreeMembership.count({where:{binaryTreeId:id,effectiveFrom:{lte:at},recordedAt:{lte:known}}});
   const items=[];
   for(const member of members.slice(0,100)){
    const [placement,ancestry,owners]=await Promise.all([
     tx.placementTreeEvidence.findFirst({where:{placementTreeEvidenceId:member.placementTreeEvidenceId,effectiveAt:{lte:at},recordedAt:{lte:known}}}),
     tx.binaryTreeAncestry.findFirst({where:{binaryTreeId:id,ancestorQualificationId:root.qualificationId,descendantQualificationId:member.qualificationId,effectiveFrom:{lte:at},recordedAt:{lte:known}}}),
     tx.qualificationOwnerInterval.findMany({where:{qualificationId:member.qualificationId,effectiveFrom:{lte:at},recordedAt:{lte:known},OR:[{effectiveTo:null},{effectiveTo:{gt:at}},{closedRecordedAt:{gt:known}}]},take:2}),
    ]);
    if(!placement||!ancestry||owners.length!==1)return {status:'UNAVAILABLE',time,items:[],nextCursor:null,total:null};
    items.push({qualificationId:member.qualificationId,parentQualificationId:placement.parentQualificationId,side:placement.side,depth:ancestry.depth,ownerType:owners[0].ownerType,activeLabel:owners[0].ownerType==='COMPANY'?'Always Active (Company Rule)':null});
   }
   return {status:'AVAILABLE',time,total,items,nextCursor:members.length>100?members[99].qualificationId:null};
  },{isolationLevel:Prisma.TransactionIsolationLevel.RepeatableRead,timeout:15000});
  await this.commands.authorize(p);return result;
 }
 private async snapshot(tx:Prisma.TransactionClient,id:string,time:AsOfContext){
  ...
  for(const slot of slots){
   ...
   const counts=await tx.$queryRaw<Array<{balls:bigint;persons:bigint;new_balls:bigint;left_balls:bigint;right_balls:bigint;left_new:bigint;right_new:bigint}>>`
    SELECT count(DISTINCT a.descendant_qualification_id) AS balls,
   ...
   const performance=slot.positionNo>=4?await readFoundingPerformance(tx,id,membership.qualificationId,time):null;
   const carry=slot.positionNo>=4?await readFoundingCarry(tx,membership.qualificationId,time):null;
   positions.push({performance,carry,leftDescendantBalls:Number(counts[0].left_balls),rightDescendantBalls:Number(counts[0].right_balls),leftNewBallsInPeriod:Number(counts[0].left_new),rightNewBallsInPeriod:Number(counts[0].right_new),positionNo:slot.positionNo,parentPositionNo:slot.parentPositionNo,side:slot.side,qualificationId:membership.qualificationId,ownerType:owner.ownerType,
    activeLabel:owner.ownerType==='COMPANY'?'Always Active (Company Rule)':null,actualSponsorSequenceNo:founding?.actualSponsorSequenceNo??(slot.positionNo===2?1:slot.positionNo===3?2:null),
    descendantBalls:Number(counts[0].balls),distinctMemberPersons:Number(counts[0].persons),newBallsInPeriod:Number(counts[0].new_balls)});
  }
  return {status:'PARTIAL',quality:'PARTIAL',finality:'NOT_APPLICABLE',time,scope:{binaryTreeId:id},definitionVersion:'TREE_FOUNDATION_V1',
   result:{binaryTreeId:id,treeCode:tree.treeCode,treeName:status.treeName,status:status.status,topologyVersion:version,economicActivation:'PENDING_MAPPING',positions,
