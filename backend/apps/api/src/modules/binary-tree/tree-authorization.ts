...
export async function authorizeTreePrincipal(db:PrismaService,p:TreePrincipal|undefined,roles:readonly string[],tx?:Prisma.TransactionClient){
  ...
  if(tx)await tx.$queryRaw`SELECT admin_access_grant_id FROM identity.admin_access_grant WHERE person_id=${p.personId}::uuid AND provider='ENTRA' AND provider_subject=${p.subject} AND role_code=${p.role} FOR SHARE`;
