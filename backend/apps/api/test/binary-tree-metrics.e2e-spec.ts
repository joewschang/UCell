...
function carry(){const row={periodEnd:new Date('2026-09-15'),ruleVersionCode:'TEST_ONLY',leftCarryOut:new Prisma.Decimal(20),rightCarryOut:new Prisma.Decimal(10)},snapshot=seal('BINARY_K1','batch',{carryRecipients:[{qualificationId:'root',leftCarryOut:'20',rightCarryOut:'10'}]},{});return {row,tx:{binaryCarry:{findMany:jest.fn(async()=>[row])},settlementBatch:{findMany:jest.fn(async()=>[{settlementBatchId:'batch',ruleVersionCode:'TEST_ONLY'}])},historicalReplaySnapshot:{findUnique:jest.fn(async()=>snapshot)},settlementRecalculationRequest:{findFirst:jest.fn(async()=>null)},replayCarryProjection:{findFirst:jest.fn(async()=>null)}}};}
describe('Tree Carry read source',()=>{
 ...
 it('uses stored replay values without recomputation',async()=>{const f=carry();f.tx.replayCarryProjection.findFirst.mockResolvedValue({sequence:7n,carry:{root:{left:'3.5',right:'0'}},stateHash:'replay-hash'} as any);expect(await readFoundingCarry(f.tx as any,'root',time)).toMatchObject({status:'AVAILABLE',value:{left:'3.5000',right:'0.0000'},replaySequence:'7'});});
