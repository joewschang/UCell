...
function fixture(){
 ...
 const db:any={authSession:{findUnique:jest.fn(async()=>session)},identityLink:{findUnique:jest.fn(async()=>({personId:pid}))},adminAccessGrant:{findMany:jest.fn(async()=>[{adminAccessGrantId:rid}])},reservoirLedgerEffect:{findUnique:jest.fn(async()=>null)},auditEvent:{create:jest.fn(async()=>({}))}};
...
describe('Admin structured Explain live finance authorization',()=>{
 it('B remains NOT_ACTIVATED without invoking any economic source',async()=>{const h=fixture();expect(await h.service.explain(h.request,'explainReservoirB',query)).toMatchObject({result:null,explainCode:'NOT_ACTIVATED',dataClassification:'FINANCE_CONFIDENTIAL'});expect(h.db.$transaction).not.toHaveBeenCalled();expect(h.db.adminAccessGrant.findMany).toHaveBeenCalledTimes(2);});
