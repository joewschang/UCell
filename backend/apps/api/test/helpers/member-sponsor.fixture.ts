...
export async function createEffectiveMemberSponsorFixture(db:PrismaClient){
 ...
 await db.identityLink.create({data:{personId:actorPerson.personId,provider:'ENTRA',providerSubject:subject}});await db.adminAccessGrant.create({data:{personId:actorPerson.personId,provider:'ENTRA',providerSubject:subject,roleCode:'SUPER_ADMIN',validFrom:new Date(Date.now()-1000)}});
 const actor:TreePrincipal={personId:actorPerson.personId,provider:'ENTRA',subject,role:'SUPER_ADMIN',sessionId:randomUUID()};const trees=new BinaryTreeService(db as any,new IdempotencyService(db as any),new OrganizationService(db as any));
