...
describe('v0.6.1 deterministic replay',()=>{
 ...
 for(const [name,amount] of [['positive delta creates compensating award',120],['negative delta creates recovery',80]] as const){
  it(name,async()=>{const e=binary(),r=e.recipients[0],before=JSON.stringify(e);const tx={...memberEconomicMocks(),entitlementReplayPosting:{findUnique:jest.fn().mockResolvedValue(null),aggregate:jest.fn().mockResolvedValue({_sum:{delta:null}}),create:jest.fn().mockImplementation(({data})=>data)},bonusAward:{create:jest.fn().mockResolvedValue({bonusAwardId:'new'})},bonusAwardLifecycleEvent:{findFirst:jest.fn().mockResolvedValue(null),create:jest.fn()},bonusRecoveryEvent:{create:jest.fn().mockResolvedValue({bonusRecoveryEventId:'recovery'})}};
 ...
 for(const [status,expected] of [['PENDING_45D','REVERSED'],['PAID','CLAWBACK']] as const) {
  it(`${status} negative historical entitlement appends ${expected} lifecycle`,async()=>{
   ...
   const tx={...memberEconomicMocks(),entitlementReplayPosting:{findUnique:jest.fn().mockResolvedValue(null),aggregate:jest.fn().mockResolvedValue({_sum:{delta:null}}),create:jest.fn().mockImplementation(({data})=>data)},bonusAward:{create:jest.fn()},
