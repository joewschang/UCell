...
export type ToolName = 'getActiveStatus' | 'explainBinaryCarry' | 'explainReservoirB';
export interface ReadDefinition {
  readonly key: string;
  readonly version: string;
  readonly name: string;
  readonly description: string;
  readonly grain: 'QUALIFICATION_PERIOD' | 'QUALIFICATION_TREE_PERIOD' | 'LEDGER_ENTRY';
...
export const READ_DEFINITIONS: Readonly<Record<ToolName, ReadDefinition>> = Object.freeze({
  getActiveStatus: definition({ ...common, key: 'active.status', name: '球的 Active 狀態',
    ...
    grain: 'QUALIFICATION_PERIOD', source: 'Active Explain adapter (not yet connected)',
