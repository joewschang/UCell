...
const terms = [
  ...
  ['CompanyBall', '公司球', 'Explicit company-owned interval; Always Active does not grant plan/rank privileges.', 'QUALIFICATION', 'QualificationOwnerInterval (not activated)'],
  ['FoundingBall', '創始位置的球', 'Occupant of per-tree canonical #4–#7; no special compensation or Active privilege.', 'QUALIFICATION', 'FoundingOccupationEvidence (not activated)'],
  ...
  ['ReservoirB', '水庫 B', 'Company final entitlement after normal Core calculation; NOT_ACTIVATED until profile gate passes.', 'EVENT', 'ReservoirBEffect (not activated)'],
  ...
  ['Tree', '二元樹', 'Independent Binary topology scope; Sponsor graph remains separate.', 'TREE', 'BinaryTree (not activated)'],
  ['MonthlyNewBalls', '當月新球數', 'First effective placement in Taipei month; transfer/company succession never creates a Ball.', 'TREE', 'PlacementTreeEvidence (not activated)'],
] as const;
...
export interface MetricDefinition {
  ...
  readonly grain: 'QUALIFICATION_PERIOD' | 'SETTLEMENT' | 'SOURCE_COHORT' | 'TREE_PERIOD';
...
const metricRows = [
  ...
  ['reservoir.b','ReservoirB','TWD','SETTLEMENT','ReservoirBEffect NOT_ACTIVATED'],
  ...
  ['tree.monthly_new_balls','MonthlyNewBalls','COUNT','TREE_PERIOD','PlacementTreeEvidence NOT_ACTIVATED'],
] as const;
export const METRIC_DEFINITIONS: Readonly<Record<string, MetricDefinition>> = Object.freeze(Object.fromEntries(metricRows.map(([key, term, unit, grain, source]) => {
  const dimensions = Object.freeze(grain === 'QUALIFICATION_PERIOD' ? ['qualificationId'] : grain === 'TREE_PERIOD' ? ['binaryTreeId'] : grain === 'SOURCE_COHORT' ? ['sourceCohortId'] : ['settlementId']);
  return [key, Object.freeze({ key, definitionVersion: '1' as const, term, unit, grain, source, dimensions,
    filters: dimensions, numerator: key === 'return.amount_rate' ? 'POSTED returned eligible recognized consideration linked to original cohort' : null,
    denominator: key === 'return.amount_rate' ? 'Original positive eligible recognized consideration of the SAME cohort and currency' : null,
    ...
    classification: (key.startsWith('reservoir.') || key.startsWith('pool.') ? 'FINANCE_CONFIDENTIAL' : 'ADMIN_OPERATIONAL') as DataClassification,
