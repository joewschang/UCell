...
export async function seedStageUat(prisma, manifest, Decimal) {
  return prisma.$transaction(async (tx) => {
    const person = await tx.person.upsert({ where: { personId: manifest.person.personId }, update: { legalName: manifest.person.legalName, status: 'EFFECTIVE', securityStatus: 'NORMAL', membershipState: 'FORMAL_MEMBER' }, create: { ...manifest.person, status: 'EFFECTIVE', securityStatus: 'NORMAL', membershipState: 'FORMAL_MEMBER' } });
