...
assert.ok(['localhost', '127.0.0.1'].includes(base.hostname), 'Shipment DB assertions only permit local PostgreSQL');
const database = `ucell_shipment_${randomUUID().replaceAll('-', '')}`;
...
function run(args) {
  ...
  assert.equal(result.status, 0, 'Shipment DB assertion child failed');
}
...
try {
  ...
  run(['scripts/shipment-persistence-db-test.mjs']);
  console.log('SHIPMENT_PERSISTENCE_DB_ISOLATED_PASS: fresh migrated database');
} finally {
  if (created) {
    await admin.$executeRawUnsafe(`DROP DATABASE "${database}" WITH (FORCE)`);
    console.log('SHIPMENT_PERSISTENCE_DB_ISOLATED_CLEANUP_PASS');
  }
