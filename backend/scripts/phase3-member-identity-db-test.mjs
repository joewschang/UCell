...
try{
 ...
 res=await call('POST','member/share-links',token,{qualificationId:ids[0]});equal(res.statusCode,201,'owned ball referral share link created');const share=res.json().data,shareUrl=new URL(share.shareUrl),shareToken=shareUrl.pathname.slice(3);equal([share.qualificationId,shareUrl.origin,shareUrl.pathname.startsWith('/r/')],[ids[0],'https://example.invalid',true],'share link binds owned ball and configured landing origin');equal(app.get(MemberShareLinkService).verify(shareToken).qualificationId,ids[0],'encrypted share token verifies to exact owned ball');equal(await db.referralLink.count({where:{tokenHash:createHash('sha256').update(shareToken).digest('hex'),referrerQualificationId:ids[0]}}),1,'share token persists only its SHA-256 digest and exact referrer ball');const publicLanding=await server.inject({method:'POST',url:'/api/v1/referrals/landing',payload:{token:shareToken}}),publicAttribution=publicLanding.json().data;equal([publicLanding.statusCode,publicAttribution.accepted,publicAttribution.referrerQualificationId,typeof publicAttribution.transitionState],[201,true,ids[0],'string'],'public referral landing records attribution and returns opaque transition state without member authentication');
 const bindHeaders={authorization:'Bearer '+token,'idempotency-key':'TEST_ONLY_REFERRAL_BIND'};let bindResponse=await server.inject({method:'POST',url:'/api/v1/member/referrals/bind',headers:bindHeaders,payload:{transitionState:publicAttribution.transitionState}});equal([bindResponse.statusCode,bindResponse.json().data.status,bindResponse.json().data.referrerQualificationId],[201,'BOUND',ids[0]],'authenticated LINE Person explicitly binds eligible anonymous attribution');bindResponse=await server.inject({method:'POST',url:'/api/v1/member/referrals/bind',headers:bindHeaders,payload:{transitionState:publicAttribution.transitionState}});equal([bindResponse.statusCode,bindResponse.json().data.replayed],[201,true],'referral Person binding lost-response retry is idempotent');equal(await db.referralAttributionHistory.count({where:{referralAttributionId:publicAttribution.referralAttributionId,action:'BOUND_TO_PERSON',boundPersonId:person.personId}}),1,'Person binding creates one append-only attribution history row');equal(await db.outboxEvent.count({where:{eventType:'REFERRAL_ATTRIBUTION_BOUND',aggregateId:publicAttribution.referralAttributionId}}),1,'Person binding emits one non-monetary outbox event');
 bindResponse=await server.inject({method:'POST',url:'/api/v1/member/referrals/bind',headers:{authorization:'Bearer '+token,'idempotency-key':'TEST_ONLY_REFERRAL_ALREADY_BOUND'},payload:{transitionState:publicAttribution.transitionState}});equal([bindResponse.statusCode,bindResponse.json().data.status],[201,'ALREADY_BOUND'],'same Person binding with a new delivery key is harmless and creates no duplicate evidence');const secondLanding=(await server.inject({method:'POST',url:'/api/v1/referrals/landing',payload:{token:shareToken}})).json().data;bindResponse=await server.inject({method:'POST',url:'/api/v1/member/referrals/bind',headers:{authorization:'Bearer '+token,'idempotency-key':'TEST_ONLY_REFERRAL_PERSON_ACTIVE_CONFLICT'},payload:{transitionState:secondLanding.transitionState}});equal([bindResponse.statusCode,bindResponse.json().code],[409,'PERSON_REFERRAL_ATTRIBUTION_CONFLICT'],'Person with another active attribution fails closed without automatic winner selection');
 let personConflict=false;try{await app.get(ReferralAttributionService).bind(other.personId,publicAttribution.transitionState,'TEST_ONLY_REFERRAL_OTHER_PERSON','TEST_ONLY_REQUEST');}catch(error){personConflict=(error?.response?.code??error?.getResponse?.()?.code)==='REFERRAL_ATTRIBUTION_PERSON_CONFLICT';}equal(personConflict,true,'attribution already bound to another Person fails closed');let tamperDenied=false;try{await app.get(ReferralAttributionService).bind(person.personId,publicAttribution.transitionState.slice(0,-1)+'A','TEST_ONLY_REFERRAL_TAMPER','TEST_ONLY_REQUEST');}catch{tamperDenied=true;}equal(tamperDenied,true,'tampered referral transition state is denied');equal((await call('POST','member/share-links',token,{qualificationId:ids[2]})).statusCode,403,'foreign ball referral share link denied');
 const shareLinks=app.get(MemberShareLinkService),attributions=app.get(ReferralAttributionService),referralAnonymousId=randomUUID(),referralAt=new Date('2026-09-17T00:00:00.000Z'),sponsorCountBefore=await db.sponsorRelationship.count();
 const firstShare=await shareLinks.create(person.personId,ids[0],referralAt),firstToken=new URL(firstShare.shareUrl).pathname.slice(3);
 let attribution=await attributions.land(firstToken,referralAnonymousId,referralAt);equal([attribution.accepted,attribution.replaced,attribution.referrerQualificationId,attribution.firstTouchAt,attribution.lockedUntil],[true,false,ids[0],referralAt.toISOString(),'2026-10-17T00:00:00.000Z'],'first valid touch creates exact 30-day provisional attribution lock');
 attribution=await attributions.land(firstToken,referralAnonymousId,new Date('2026-09-17T00:30:00.000Z'));equal([attribution.reasonCode,attribution.lockedUntil,attribution.lastTouchAt],['SAME_REFERRER_LOCK_UNCHANGED','2026-10-17T00:00:00.000Z','2026-09-17T00:30:00.000Z'],'same referrer touch updates last touch without extending lock');
 const competingShare=await shareLinks.create(person.personId,ids[1],new Date('2026-09-17T00:40:00.000Z')),competingToken=new URL(competingShare.shareUrl).pathname.slice(3);
 attribution=await attributions.land(competingToken,referralAnonymousId,new Date('2026-09-17T00:45:00.000Z'));equal([attribution.accepted,attribution.referrerQualificationId,attribution.reasonCode],[false,ids[0],'ATTRIBUTION_LOCK_ACTIVE'],'competing referrer inside lock records evidence without replacement');
 const replacementShare=await shareLinks.create(person.personId,ids[1],new Date('2026-10-17T00:00:00.000Z')),replacementToken=new URL(replacementShare.shareUrl).pathname.slice(3);
 attribution=await attributions.land(replacementToken,referralAnonymousId,new Date('2026-10-17T00:00:00.000Z'));equal([attribution.accepted,attribution.replaced,attribution.referrerQualificationId,attribution.firstTouchAt,attribution.lockedUntil],[true,true,ids[1],'2026-10-17T00:00:00.000Z','2026-11-16T00:00:00.000Z'],'first competing touch at lock boundary replaces attribution and starts a new 30-day lock');
 equal((await db.referralAttributionHistory.findMany({where:{referralAttributionId:attribution.referralAttributionId},orderBy:{occurredAt:'asc'}})).map(row=>[row.action,row.reasonCode]),[['CREATED','FIRST_VALID_TOUCH'],['TOUCH_RECORDED','SAME_REFERRER_LOCK_UNCHANGED'],['COMPETING_TOUCH_RECORDED','ATTRIBUTION_LOCK_ACTIVE'],['REPLACED','ATTRIBUTION_LOCK_EXPIRED']],'attribution history preserves all accepted and competing touch evidence');
 equal(await db.sponsorRelationship.count(),sponsorCountBefore,'referral attribution never creates or rewrites Sponsor relationship');
 ...
 equal(balls.map(ball=>ball.bonuses.data.awards.map(award=>[award.name,award.status,award.amount])),[[['EPV','PENDING',null]],[['EPV','PENDING',null]]],'unfinalized Bonus entitlement remains pending/null and isolated by ball');
 ...
 equal((await call('GET','member/bonuses?qualificationId='+ids[0]+'&period=2026-12',token)).json().data.awards.map(a=>[a.status,a.amount]),[['PENDING',null]],'unfinalized lifecycle remains pending/null regardless of reverse UUID order');
