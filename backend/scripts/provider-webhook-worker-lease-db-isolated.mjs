...
assert.ok(['localhost', '127.0.0.1'].includes(base.hostname), 'Worker lease assertions only permit local PostgreSQL');
const database = `ucell_provider_worker_${randomUUID().replaceAll('-', '')}`;
...
function run(args) {
  ...
  assert.equal(result.status, 0, 'Provider webhook worker lease assertion child failed');
}
let created = false;
try {
  ...
  run(['scripts/provider-webhook-worker-lease-db-test.mjs']);
  console.log('PROVIDER_WEBHOOK_WORKER_LEASE_DB_ISOLATED_PASS: fresh migrated database');
} finally {
  if (created) { await admin.$executeRawUnsafe(`DROP DATABASE "${database}" WITH (FORCE)`); console.log('PROVIDER_WEBHOOK_WORKER_LEASE_DB_ISOLATED_CLEANUP_PASS'); }
