...
(async()=>{
 ...
 try {
  ...
  const balls=['q1','q2'].map(id=>({id,code:id,rank:'ELITE',active:true,ballLabel:id}));
  let pending;
  ...
  await page.getByText('正在確認 q2｜q2，請稍候…',{exact:true}).waitFor();
  ...
  await page.getByText('正在確認 q1｜q1，請稍候…',{exact:true}).waitFor();
