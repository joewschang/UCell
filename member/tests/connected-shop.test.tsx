...
it('renders a real empty catalog and disables checkout',async()=>{
 vi.stubGlobal('fetch',vi.fn(async(url:string)=>url.endsWith('/delivery-profile')?envelope(delivery):envelope([])));await mount();expect(JSON.stringify(tree.toJSON())).toContain('目前沒有上架商品');expect(button('建立待付款訂單').props.disabled).toBe(true);
});
it.each([
