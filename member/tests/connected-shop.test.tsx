...
it('uses Core order amount and requests authorized detail; submits no price or PV',async()=>{
 const fetch=vi.fn(async(url:string,init:RequestInit)=>url.endsWith('/products')?envelope([{id:'p1',name:'Core product',price:999999,pv:null,available:true}]):envelope(order));vi.stubGlobal('fetch',fetch);
 await mount();act(()=>button('加入購物車').props.onClick());await act(async()=>{await button('建立待付款訂單').props.onClick();});
...
it('preserves cart and key through a retryable 409 without assuming order success',async()=>{
 let posts=0;const fetch=vi.fn(async(url:string,init:RequestInit)=>{
  if(url.endsWith('/products'))return envelope([{id:'p1',name:'Core product',price:399,pv:null,available:true}]);
  if(init.method==='POST'&&posts++===0)return {ok:false,status:409,json:async()=>({code:'RETRYABLE_CONFLICT'})};
  return envelope(order);
 });vi.stubGlobal('fetch',fetch);await mount();act(()=>button('加入購物車').props.onClick());
 await act(async()=>{await button('建立待付款訂單').props.onClick();});expect(JSON.stringify(tree.toJSON())).toContain('保留原資料重試');
 expect(JSON.stringify(tree.toJSON())).not.toContain('待付款訂單已建立');
 await act(async()=>{await button('建立待付款訂單').props.onClick();});const calls=fetch.mock.calls.filter(([,init])=>init.method==='POST');
...
it('renders a real empty catalog and disables checkout',async()=>{
 vi.stubGlobal('fetch',vi.fn().mockResolvedValue(envelope([])));await mount();expect(JSON.stringify(tree.toJSON())).toContain('目前沒有上架商品');expect(button('建立待付款訂單').props.disabled).toBe(true);
});
it.each([
 ...
 vi.stubGlobal('fetch',vi.fn(async(url:string)=>url.endsWith('/products')?envelope([{id:'p1',name:'Core product',price:399,pv:null,available:true}]):{ok:false,status,json:async()=>({code})}));
 await mount();act(()=>button('加入購物車').props.onClick());await act(async()=>{await button('建立待付款訂單').props.onClick();});
 expect(JSON.stringify(tree.toJSON())).toContain(message);expect(JSON.stringify(tree.toJSON())).not.toContain('待付款訂單已建立');expect(button('建立待付款訂單').props.disabled).toBe(false);
});
