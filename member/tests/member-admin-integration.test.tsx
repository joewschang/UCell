...
function qualification(b:any){return {id:b.qualificationId,code:'INTEGRATION BALL',rank:'STARTER',active:true,ballLabel:'球1'};}
...
it('refreshes the current-ball order list after an Admin payment, replacing stale status',async()=>{
  ...
  const fetch=vi.fn(async(url:string)=>new Response(JSON.stringify(url.includes('/qualifications')?{...b.memberOrderAfter,data:[q]}:paid?b.memberOrdersAfter:b.memberOrdersBefore)));
  ...
  const paidCard=tree!.root.findAllByType('article').find(article=>article.findByType('h3').children.join('')===b.orderCreate.data.id)!;
  expect(paidCard.findAllByType('p').map(p=>p.children.join(''))).toContain('訂單：PAID');
