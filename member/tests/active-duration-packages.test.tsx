...
const q={id:qualificationId,code:'BALL000001',rank:'STARTER',active:true,ballLabel:'球 1'};
...
it('binds the selected Ball to an active-duration package order',async()=>{
 ...
 const post=fetch.mock.calls.find(([,init])=>init.method==='POST')!;expect(JSON.parse(post[1].body as string)).toEqual({packageVersionId,targetQualificationId:qualificationId,selections:[{productRuleProfileId,quantity:1}]});expect(JSON.stringify(tree.toJSON())).toContain('目標資格');expect(JSON.stringify(tree.toJSON())).toContain('BALL000001');expect(JSON.stringify(tree.toJSON())).not.toContain(qualificationId);
});
