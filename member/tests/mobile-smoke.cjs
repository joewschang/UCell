...
(async()=>{
 ...
 try {
  ...
  await page.getByRole('heading',{name:'肽時光膠原飲 TIP-777'}).waitFor();
  assert.equal(await page.getByRole('img',{name:/商品包裝/}).count(),5);
  if(process.env.SMOKE_SCREENSHOT) await page.screenshot({path:process.env.SMOKE_SCREENSHOT.replace(/\.png$/,'.products.png'),fullPage:true,animations:'disabled'});
