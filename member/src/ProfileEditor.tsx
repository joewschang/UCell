...
export default function ProfileEditor({refresh}:{refresh:()=>void}){
 ...
 const pending=useRef<{body:string;key:string}|null>(null),inFlight=useRef(false);
 ...
 async function submit(e:React.FormEvent){
  ...
  try{await updateProfile(input,pending.current.key);pending.current=null;setName('');setEmail('');setPhone('');setMessage('會員資料已更新');refresh();}
