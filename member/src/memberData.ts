...
export type ConnectedOrder={qualificationId:string|null;id:string;orderNo:string;status:string;total:string;createdAt:string;lines:{productId:string;name:string;quantity:string;unitPrice?:string;amount:string}[]};
function parseConnectedOrder(value:unknown,q:Qualification):ConnectedOrder{
 const row=value as ConnectedOrder;
 if(!row||row.qualificationId!==q.id||typeof row.id!=='string'||!/^\d+$/.test(row.orderNo)||typeof row.total!=='string'||!/^\d+(\.\d+)?$/.test(row.total)||typeof row.status!=='string'||!Array.isArray(row.lines)||!row.lines.every(line=>typeof line.name==='string'&&typeof line.quantity==='string'&&typeof line.amount==='string'))throw new Error('訂單格式或資格不符，已停止顯示');
 return row;
}
...
export async function getPendingPlacements(signal:AbortSignal):Promise<PendingPlacement[]>{
 if(isMock)return [];
 const rows=await api<unknown>('/member/placements/pending',{signal});
 if(!Array.isArray(rows)||!rows.every((value:any)=>value&&(value.pendingBallNo===null||(typeof value.pendingBallNo==='string'&&ballNo.test(value.pendingBallNo)))&&typeof value.placementReference==='string'&&/^P[1-9]\d{0,18}$/.test(value.placementReference)&&nullableString(value.pendingMemberNo)&&typeof value.packageType==='string'&&nullableString(value.sponsorBallNo)&&value.paymentState==='CONFIRMED'&&nullableDate(value.requestedAt)&&nullableDate(value.dueAt)&&nullableDate(value.placedAt)&&['PLACEMENT_PENDING','PLACEMENT_OVERDUE'].includes(value.status)&&['0_24H','24_48H','48_72H','OVERDUE'].includes(value.agingBucket)&&typeof value.policyVersion==='string'&&!Object.prototype.hasOwnProperty.call(value,'qualificationId')))throw new Error('待安置資料格式異常，已停止顯示');
 return rows as PendingPlacement[];
}
export async function placePendingQualification(placementReference:string,binaryParentBallNo:string,side:'LEFT'|'RIGHT',key:string){
 if(!/^P[1-9]\d{0,18}$/.test(placementReference)||!ballNo.test(binaryParentBallNo))throw new Error('球編號格式不正確');
 const row=await api<unknown>(`/member/placements/pending/${encodeURIComponent(placementReference)}/place`,{method:'POST',headers:{'Idempotency-Key':key},body:JSON.stringify({binaryParentBallNo,side})}) as Record<string,unknown>;
 if(!row||typeof row.ballNo!=='string'||!ballNo.test(row.ballNo)||row.binaryParentBallNo!==binaryParentBallNo||row.side!==side||typeof row.placedAt!=='string'||!Number.isFinite(Date.parse(row.placedAt))||typeof row.evidenceHash!=='string'||!hash.test(row.evidenceHash))throw new Error('安置結果格式異常，已停止顯示');
 return row;
}
...
export type SponsorCandidate={kind:'BALL';sponsorBallNo:string;planLevelCode:string;effectiveAt:string;ruleVersion:string}|{kind:'COMPANY_ALIAS';displayLabel:string;policyVersion:string;effectiveAt:string;ruleVersion:string};
...
export async function resolveSponsorCandidate(code:string,signal:AbortSignal):Promise<SponsorCandidate>{if(!/^[A-Z][A-Z0-9_-]{1,39}$/.test(code))throw new Error('推薦碼格式不正確');const row=await api<unknown>('/onboarding/sponsor/candidate?'+new URLSearchParams({code}),{signal}) as SponsorCandidate;if(!row||row.ruleVersion!=='R1.0B'||typeof row.effectiveAt!=='string'||!Number.isFinite(Date.parse(row.effectiveAt)))throw new Error('推薦碼無法核驗');if(row.kind==='BALL'&&row.sponsorBallNo===code&&typeof row.planLevelCode==='string'&&row.planLevelCode)return row;if(row.kind==='COMPANY_ALIAS'&&typeof row.displayLabel==='string'&&row.displayLabel&&typeof row.policyVersion==='string'&&row.policyVersion)return row;throw new Error('推薦碼無法核驗');}
...
export async function createPackageOrder(packageVersionId:string,selections:{productRuleProfileId:string;quantity:number}[],key:string,targetQualificationId?:string,sponsorCode?:string){if(!uuid.test(packageVersionId)||targetQualificationId!==undefined&&!uuid.test(targetQualificationId)||sponsorCode!==undefined&&!/^[A-Z][A-Z0-9_-]{1,39}$/.test(sponsorCode)||!selections.length||new Set(selections.map(x=>x.productRuleProfileId)).size!==selections.length||!selections.every(x=>uuid.test(x.productRuleProfileId)&&Number.isSafeInteger(x.quantity)&&x.quantity>0))throw new Error('套組選擇資料格式異常');const result=await api<{qualificationId:string;id:string;status:string;total:string;paymentStatus:string;shipmentStatus:string;createdAt:string;replayed:boolean;lines:unknown[]}>('/member/orders',{method:'POST',headers:{'Idempotency-Key':key},body:JSON.stringify({packageVersionId,...(targetQualificationId?{targetQualificationId}:{}),...(sponsorCode?{sponsorCode}:{}),selections})});if(!result||!uuid.test(result.qualificationId)||targetQualificationId!==undefined&&result.qualificationId!==targetQualificationId||!uuid.test(result.id)||result.status!=='CONFIRMED'||!decimal.test(result.total)||result.paymentStatus!=='PENDING'||!Number.isFinite(Date.parse(result.createdAt))||typeof result.replayed!=='boolean'||!Array.isArray(result.lines))throw new Error('套組訂單回應異常，已停止顯示');return result;}
