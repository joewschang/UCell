...
export async function resolveSponsorCandidate(code:string,signal:AbortSignal):Promise<SponsorCandidate>{if(!/^[A-Z][A-Z0-9_-]{0,39}(?:X\d{6,}|\d{6,})$/.test(code))throw new Error('推薦球編號格式不正確');const row=await api<unknown>('/onboarding/sponsor/candidate?'+new URLSearchParams({code}),{signal}) as SponsorCandidate;if(!row||row.sponsorBallNo!==code||typeof row.planLevelCode!=='string'||!row.planLevelCode||row.ruleVersion!=='R1.0B'||typeof row.effectiveAt!=='string'||!Number.isFinite(Date.parse(row.effectiveAt)))throw new Error('推薦球編號無法核驗');return row;}
