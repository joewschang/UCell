...
export const getQualifications = () => isMock ? Promise.resolve(qualifications) : api<Qualification[]>('/member/qualifications');
export const getPerson = (signal: AbortSignal) => isMock ? Promise.resolve<Person>({ name: '示範會員', memberNo: 'DEMO-000001', email: null, phone: null }) : api<Person>('/member/me', { signal });
async function scoped<T extends Scoped>(path: string, q: Qualification, sample: T, signal: AbortSignal, period?: string): Promise<T> {
    if (isMock)
        return sample;
...
export const getOrganization = (q: Qualification, s: AbortSignal) => scoped<Organization>('organization/sponsor', q, { qualificationId: q.id, sponsor: { code: q.id === 'q1' ? 'DEMO-S01' : 'DEMO-S02', name: '示範推薦人' }, referrals: q.id === 'q1' ? [{ code: 'DEMO-R01', name: '示範直推會員' }] : [] }, s);
export const getBinary = (q: Qualification, s: AbortSignal) => scoped<Binary>('organization/binary', q, { qualificationId: q.id, left: { count: q.id === 'q1' ? 3 : 0, volume: null }, right: { count: q.id === 'q1' ? 2 : 0, volume: null } }, s);
