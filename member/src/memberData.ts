...
export type PendingPlacement={pendingBallNo:string;pendingMemberNo:string|null;packageType:string;sponsorBallNo:string|null;paymentState:'CONFIRMED';requestedAt:string|null;dueAt:string|null;placedAt:string|null;status:'PLACEMENT_PENDING'|'PLACEMENT_OVERDUE';agingBucket:'0_24H'|'24_48H'|'48_72H'|'OVERDUE';policyVersion:string};
const ballNo=/^[A-Z][A-Z0-9]{5,58}$/;
export async function getPendingPlacements(signal:AbortSignal):Promise<PendingPlacement[]>{
 ...
 if(!Array.isArray(rows)||!rows.every((value:any)=>value&&ballNo.test(value.pendingBallNo)&&nullableString(value.pendingMemberNo)&&typeof value.packageType==='string'&&nullableString(value.sponsorBallNo)&&value.paymentState==='CONFIRMED'&&nullableDate(value.requestedAt)&&nullableDate(value.dueAt)&&nullableDate(value.placedAt)&&['PLACEMENT_PENDING','PLACEMENT_OVERDUE'].includes(value.status)&&['0_24H','24_48H','48_72H','OVERDUE'].includes(value.agingBucket)&&typeof value.policyVersion==='string'&&!Object.prototype.hasOwnProperty.call(value,'qualificationId')))throw new Error('待安置資料格式異常，已停止顯示');
 return rows as PendingPlacement[];
}
export async function placePendingQualification(ballNoValue:string,binaryParentBallNo:string,side:'LEFT'|'RIGHT',key:string){
 if(!ballNo.test(ballNoValue)||!ballNo.test(binaryParentBallNo))throw new Error('球編號格式不正確');
 const row=await api<unknown>(`/member/placements/${encodeURIComponent(ballNoValue)}/place`,{method:'POST',headers:{'Idempotency-Key':key},body:JSON.stringify({binaryParentBallNo,side})}) as Record<string,unknown>;
 if(!row||row.ballNo!==ballNoValue||row.binaryParentBallNo!==binaryParentBallNo||row.side!==side||typeof row.placedAt!=='string'||!Number.isFinite(Date.parse(row.placedAt))||typeof row.evidenceHash!=='string'||!hash.test(row.evidenceHash))throw new Error('安置結果格式異常，已停止顯示');
 return row;
}
