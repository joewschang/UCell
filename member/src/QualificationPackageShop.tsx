...
import {pendingSponsorCandidate} from './liff';
...
export function PackageSelection({offer,onCancel,onCreated,targetQualificationId}:{offer:PackageOffer;onCancel:()=>void;onCreated:()=>void;targetQualificationId?:string}){
 ...
 async function submit(){const selections=Object.entries(quantities).filter(([,quantity])=>quantity>0).sort(([a],[b])=>a.localeCompare(b)).map(([productRuleProfileId,quantity])=>({productRuleProfileId,quantity})),sponsorCode=pendingSponsorCandidate();if(flight.current||count!==offer.selectableProductQuantity)return;const body=JSON.stringify({packageVersionId:offer.packageVersionId,targetQualificationId,sponsorCode,selections});if(pending.current?.body!==body)pending.current={body,key:crypto.randomUUID()};flight.current=true;setBusy(true);setError('');try{const order=await createPackageOrder(offer.packageVersionId,selections,pending.current.key,targetQualificationId,sponsorCode);setReceipt({id:order.id,qualificationId:order.qualificationId,total:order.total});pending.current=null;onCreated();}catch(e){setError(e instanceof Error?e.message:'建立套組訂單失敗，請保留選擇後重試');}finally{flight.current=false;setBusy(false);}}
