...
function Home({ q }: {
    ...
    return <Result state={state}>{d => <><section className="hero uc-member-identity"><small>會員中心</small><h2>您好，{d.memberName}</h2><p>{d.memberNo} · {q.code}｜{q.ballLabel}</p></section><section className="card uc-member-status"><div><span>資格狀態</span><strong>{q.active?'資格活躍':'資格未活躍'}</strong><small>{d.monthReference??'月份未提供'} · {d.activeInterval?`${d.activeInterval.activeFrom} ～ ${d.activeInterval.activeTo}`:'Active 區間 unavailable'}</small></div><div><span>本月重購</span><h2>{{ ACTIVE: '已完成', PENDING: '確認中', INACTIVE: '未完成' }[d.monthlyRepurchaseStatus]}</h2></div></section><h3 className="uc-eyebrow">業績摘要</h3><Metrics items={[["PV", d.pv], ["RPV", d.rpv], ["EPV", d.epv]]}/><section className="card uc-bonus-summary"><span>本期獎金 · {statusNames[d.bonusStatus] ?? d.bonusStatus}</span><h2><MoneyState amount={d.bonusAmount} status={d.bonusAmount===null?'PENDING':d.bonusStatus}/></h2></section><details className="uc-repurchase-expander"><summary>查看重購認列詳情</summary><RepurchaseDetails q={q}/></details><h3>快速服務</h3><MobileActionGrid>{[['/organization', '我的組織'], ['/performance', '我的業績'], ['/bonuses', '獎金明細'], ['/shop', '商品商城'], ['/orders', '我的訂單'], ['/content', '影音內容'], ['/me', '會員資料']].map(([path, title]) => <Link key={path} to={path}>{title}</Link>)}</MobileActionGrid></>}</Result>;
}
...
function Bonuses({ q }: {
    ...
    return <><div className="uc-member-title"><h2>獎金明細</h2><QualificationBadge code={q.code} ball={q.ballLabel} rank={data.displayRank(q.rank)}/></div><Period value={period} onChange={setPeriod}/><Result state={awards}>{d => d.awards.length ? d.awards.map(a => <article className="card" key={a.id}><div><h3>{a.name}</h3><p className="uc-award-status">{statusNames[a.status] ?? a.status}</p></div><MoneyState amount={a.finalAmount??null} status={a.finalAmount==null?'PENDING':a.status}/><AwardLifecycle status={a.status}/><details className="uc-domain-detail"><summary>查看制度明細</summary><p>Domain status：{a.status}</p><p>資格：{q.code}｜{q.ballLabel}</p><p>Theory：{a.theoryAmount==null?'unavailable':money(a.theoryAmount)}</p><p>Final：{a.finalAmount==null?'unavailable':money(a.finalAmount)}</p><p>Payable：{a.payableAmount==null?'unavailable':money(a.payableAmount)}</p><p>Settlement：{a.settlementStatus??'unavailable'}{a.pendingReason?` · ${a.pendingReason}`:''}</p><p>Settlement / nominal / adjusted：{a.settlementDate??'unavailable'} / {a.nominalPayoutDate??'unavailable'} / {a.adjustedPayoutDate??'unavailable'}</p><p>Calendar：{a.businessCalendarVersion??'unavailable'} · Rule：{a.ruleVersion??'unavailable'} · Hash：{a.parameterSnapshotHash??'unavailable'}</p></details></article>) : <EmptyState title="此月份尚無獎金紀錄"/>}</Result><h3>入帳與調整紀錄</h3><Result state={ledger}>{d => d.entries.length ? d.entries.map(e => <article className="card" key={e.id}><h3>{e.label}</h3><strong>{money(e.amount)}</strong><p>{e.postedAt}</p><small>來源：{e.sourceId}</small></article>) : <EmptyState title="此月份尚無入帳紀錄"/>}</Result></>;
}
function Orders({ q }: {
    q: Qualification;
...
function Me() { const state = useResource('person', data.getPerson); const { qualifications } = useQualification(); return <><MemberPageHeader title="我的帳戶"/><Result state={state}>{p => <><section className="card"><h3>{p.name}</h3><p>{p.memberNo}</p><p>會員狀態：{p.membershipState??'尚未完成網路會員註冊'}</p><p>電子郵件：{p.email ?? '未提供'}</p><p>電話：{p.phone ?? '未提供'}</p></section>{!data.isMock&&p.membershipState===null&&<NetworkRegistration person={p} refresh={state.retry}/>} {!data.isMock&&['NETWORK_MEMBER','FORMAL_PENDING'].includes(p.membershipState??'')&&<FormalUpgrade/>}</>}</Result><ProfileEditor refresh={state.retry}/><h3>我的資格</h3>{qualifications.map(q => <article key={q.id} className="card"><strong>{q.code} · {q.ballLabel}</strong><p>{data.displayRank(q.rank)} · {q.active ? '活躍' : '未活躍'}</p></article>)}<EndSession connected={!data.isMock}/></>; }
function MemberApp() {
