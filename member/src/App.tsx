...
function Shop() { const state = useResource('products', data.getProducts); return <><h2>商品商城</h2><p>線上購買尚未開放，價格與 PV 以訂單確認資料為準。</p><Result state={state}>{products => products.length ? products.map(p => <article className="card" key={p.id}><div className="product-mark" aria-hidden="true">UCell</div><h3>{p.name}</h3><p>{money(p.price)} · PV {number(p.pv)}</p><button disabled>購買功能準備中</button></article>) : <p>目前沒有上架商品</p>}</Result><Link className="text-link" to="/orders">查看我的訂單 →</Link></>; }
function Orders({ q }: {
    q: Qualification;
}) { const state = useResource(`orders:${q.id}`, s => data.getOrders(q, s)); const [expanded, setExpanded] = useState<string | null>(null); return <><h2>我的訂單</h2><Result state={state}>{d => d.orders.length ? d.orders.map(o => <article className="card" key={o.id}><h3>{o.id}</h3><p>{o.createdAt} · {money(o.total)}</p><p>訂單：{o.status}</p><button aria-expanded={expanded === o.id} onClick={() => setExpanded(expanded === o.id ? null : o.id)}>付款與配送狀態</button>{expanded === o.id && <div><p>付款：{o.paymentStatus}</p><p>配送：{o.shipmentStatus}</p></div>}</article>) : <p>此資格目前沒有訂單</p>}</Result></>; }
