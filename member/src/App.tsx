...
function Organization({ q }: {
    ...
    return <><MemberPageHeader title="我的組織" q={q}/><div className="tabs"><button aria-pressed={tab === 'sponsor'} onClick={() => setTab('sponsor')}>推薦組織</button><button aria-pressed={tab === 'binary'} onClick={() => setTab('binary')}>二元安置組織</button></div>{tab === 'sponsor' ? <><Result state={sponsor}>{d => <><section className="card"><h3>推薦摘要</h3><p className="uc-eyebrow">我的推薦人</p><p>{d.sponsor ? `${d.sponsor.name} · ${d.sponsor.code}` : '目前沒有推薦人資料'}</p></section><section className="card"><h3>直推會員</h3><p className="uc-system-note">本次 API 回傳 {d.referrals.length} 位直推會員</p>{d.referrals.length ? d.referrals.map(r => <p key={r.code}>{r.name} · {r.code}</p>) : <p>目前沒有直推會員</p>}</section></>}</Result><ReferralShare q={q}/></> : <Result state={binary}>{d => <><p>左、右區屬二元安置組織，與推薦組織分別呈現。</p><div className="uc-binary-summary">{[['左區',d.left],['右區',d.right]].map(([label,side])=><section className="card" key={String(label)}><h3>{String(label)}</h3><dl><dt>{String(label)}人數</dt><dd>{number((side as typeof d.left).count)}</dd><dt>{String(label)}業績（Core 回傳）</dt><dd>{d.settlementMetrics.status === 'AVAILABLE' ? number((side as typeof d.left).volume) : availabilityText.readModelMissing}</dd><dt>Carry</dt><dd>{d.settlementMetrics.status === 'AVAILABLE' ? number((side as typeof d.left).carry) : availabilityText.readModelMissing}</dd></dl></section>)}</div><section className="card"><h3>完整二元安置組織</h3><p>{d.fullTree.status === 'UNAVAILABLE' ? `${availabilityText.readModelMissing}：完整組織檢視尚未提供。` : '完整組織資料已可使用。'}</p></section></>}</Result>}</>;
}
...
function Bonuses({ q }: {
    ...
    return <><MemberPageHeader title="獎金明細" q={q}/><Period value={period} onChange={setPeriod}/><Result state={awards}>{d => d.awards.length ? d.awards.map(a => <article className="card" key={a.id}><div><h3>{a.name}</h3><p className="uc-award-status">{awardStatusLabels[a.status]}</p></div><MoneyState amount={a.finalAmount??null} status={a.finalAmount==null?'PENDING':a.status}/><AwardLifecycle status={a.status}/><details className="uc-domain-detail"><summary>查看制度明細</summary><p>Domain status：{a.status}</p><p>經營資格：{q.code}｜{q.ballLabel}</p><p>Theory：{money(a.theoryAmount)}</p><p>Final：{money(a.finalAmount)}</p><p>Payable：{money(a.payableAmount)}</p><p>Settlement：{a.settlementStatus??availabilityText.fieldMissing}{a.pendingReason?` · ${a.pendingReason}`:''}</p><p>Settlement / nominal / adjusted：{a.settlementDate??availabilityText.fieldMissing} / {a.nominalPayoutDate??availabilityText.fieldMissing} / {a.adjustedPayoutDate??availabilityText.fieldMissing}</p><p>Calendar：{a.businessCalendarVersion??availabilityText.fieldMissing} · Rule：{a.ruleVersion??availabilityText.fieldMissing} · Hash：{a.parameterSnapshotHash??availabilityText.fieldMissing}</p></details></article>) : <EmptyState title="此月份尚無獎金紀錄"/>}</Result><h3>入帳與調整紀錄</h3><Result state={ledger}>{d => d.entries.length ? d.entries.map(e => <article className="card" key={e.id}><h3>{e.label}</h3><strong>{money(e.amount)}</strong><p>{e.postedAt}</p><small>來源：{e.sourceId}</small></article>) : <EmptyState title="此月份尚無入帳紀錄"/>}</Result></>;
}
function Orders({ q }: {
    q: Qualification;
