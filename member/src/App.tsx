...
import {MemberAppShell,QualificationSwitcher,MobileActionGrid,MetricCard,MoneyState,LoadingState,ErrorState,AwardLifecycle,EmptyState} from '@ucell/design-system';
...
function ContextBar({beforeSelect}:{beforeSelect:()=>void}) { const {qualifications,current,select,feedback}=useQualification();return <QualificationSwitcher options={qualifications.map(q=>({id:q.id,label:q.code+'｜'+data.displayRank(q.rank)+'｜'+q.ballLabel}))} value={current?.id??''} onChange={id=>{beforeSelect();select(id)}} feedback={feedback} active={current?.active??false}/>; }
function Home({ q }: {
    q: Qualification;
}) {
...
function MemberApp() {
    const { loading, loadingLabel, error, retry, current } = useQualification();
    const { unread } = useNotifications(current?.id);
    const {mainRef,begin}=useQualificationFocus(loading,error,current?.id);
    return <MemberAppShell><a className="uc-skip-link" href="#member-main">跳至主要內容</a><header><div><b>UCell</b><small>會員服務</small></div>{current && <Link className="notification-link" to="/notifications" aria-label={data.isMock ? `通知中心，${unread} 則未讀` : '通知中心'}>通知{data.isMock ? ` ${unread}` : ''}</Link>}</header>{data.isMock && <aside className="demo-banner" data-environment="mock-visual-only"><strong>DEV 示範模式 · </strong><span>目前為示範資料，不代表真實業績、獎金或訂單。</span></aside>}<main ref={mainRef} id="member-main" tabIndex={-1}>{loading ? <LoadingState label={loadingLabel}/> : error ? <ErrorState message={error} retry={()=>{begin();retry()}}/> : current ? <><ContextBar beforeSelect={begin}/><div key={current.id}><Routes><Route path="/" element={<Home q={current}/>}/><Route path="/organization" element={<Organization q={current}/>}/><Route path="/performance" element={<Performance q={current}/>}/><Route path="/bonuses" element={<Bonuses q={current}/>}/><Route path="/shop" element={<Shop q={current}/>}/><Route path="/orders" element={<Orders q={current}/>}/><Route path="/content" element={<ContentList q={current}/>}/><Route path="/content/:id" element={<ContentDetail q={current}/>}/><Route path="/me" element={<Me />}/><Route path="/notifications" element={<Notifications q={current}/>}/><Route path="*" element={<section className="card"><h2>找不到頁面</h2><Link to="/">返回首頁</Link></section>}/></Routes></div></> : <Routes><Route path="/shop" element={<QualificationPackageShop onCreated={retry}/>}/><Route path="/me" element={<Me/>}/><Route path="*" element={<section className="card"><h2>尚未取得會員資格</h2><p>可先完成正式會員資料，再透過正式套組取得第一個會員資格。</p><Link className="text-link" to="/shop">選擇正式會員套組</Link><button onClick={retry}>重新查詢</button><Link className="text-link" to="/me">查看會員資料</Link><EndSession connected={!data.isMock}/></section>}/></Routes>}</main><MemberNavigation/></MemberAppShell>;
}
