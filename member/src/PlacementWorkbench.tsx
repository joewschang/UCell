...
export default function PlacementWorkbench(){
 ...
 return <section aria-label="推薦人待安置工作台"><h3>待安置工作台</h3><p>只顯示您持有推薦球且已付款確認的項目。送出後由伺服器再次驗證推薦權限、二元位置與制度規則。</p>{state.data.length?state.data.map(row=><PlacementCard key={row.pendingBallNo} row={row} onDone={state.retry}/>):<EmptyState title="目前沒有待安置項目"/>}</section>;
}
