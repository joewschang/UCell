...
export function QualificationDetail({id}:{id:string}){
 ...
 let body;
 if(tab==='Overview')body=<dl className="detail-grid"><dt>Qualification ID</dt><dd>{id}</dd><dt>持有主體</dt><dd>{q.currentCompanyPrincipalId?'公司主體：'+q.currentCompanyPrincipalId:q.currentHolderPersonId}</dd><dt>Plan</dt><dd>{q.planLevelCode}</dd><dt>Domain Status</dt><dd>{q.status}</dd><dt>Active Flag</dt><dd>{q.currentCompanyPrincipalId?'Always Active (Company Rule)':q.activeFlag?'ACTIVE':'INACTIVE'}</dd><dt>Effective</dt><dd>{dateTime(q.effectiveAt)}</dd></dl>;
 ...
 else if(tab==='Active')body=q.currentCompanyPrincipalId?<p>Always Active (Company Rule) · 公司持有期間適用；不代表公司金額功能已啟用。</p>:q.activePeriods?.length?<><p className="uc-muted">Backend 最近最多 20 筆 Active history</p><ol className="timeline-list">{q.activePeriods.map((p:any)=><li key={p.activePeriodId}>{dateTime(p.activeFrom)} → {dateTime(p.activeTo)} · {p.sourceType}</li>)}</ol></>:<EmptyState title="尚無 Active history"/>;
 ...
 return <section className="uc-qualification-detail"><QualificationBadge code={'Q#'+qNo(q.qualificationNo)} rank={q.planLevelCode??'方案尚未綁定'}/><div className="uc-detail-tabs" role="tablist" aria-label="Qualification 詳情" onKeyDown={e=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(e.key))return;e.preventDefault();const i=tabs.indexOf(tab);const next=e.key==='Home'?0:e.key==='End'?tabs.length-1:(i+(e.key==='ArrowRight'?1:-1)+tabs.length)%tabs.length;setTab(tabs[next]);document.getElementById(group+'-'+next)?.focus()}}>{tabs.map((t,i)=><button key={t} id={group+'-'+i} role="tab" aria-selected={tab===t} aria-controls={group+'-panel'} tabIndex={tab===t?0:-1} onClick={()=>setTab(t)}>{t}</button>)}</div><div id={group+'-panel'} role="tabpanel" aria-labelledby={group+'-'+tabs.indexOf(tab)}>{body}</div></section>
}
