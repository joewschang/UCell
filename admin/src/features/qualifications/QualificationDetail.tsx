...
function companyOwner(qualification:any){const owner=admin360(qualification)?.owner;return owner?owner.status==='AVAILABLE'&&owner.ownerType==='COMPANY':Boolean(qualification?.currentCompanyPrincipalId);}
...
export function QualificationDetail({id}:{id:string}){
 ...
 const companyHeld=companyOwner(q),plan=planLabel(q),owner=ownerLabel(q),tree=treeLabel(q),position=positionLabel(q),path=pathLabel(q),rank=rankLabel(q);
 const planSource=admin360(q)?.plan;
 let body:React.ReactNode;
 if(tab==='Overview')body=<><dl className="detail-grid"><dt>Ball Number（球編號）</dt><dd>{q.ballNo??'尚未完成 Binary 放置'}</dd><dt>Tree Code（樹號）</dt><dd>{tree}</dd><dt>Binary Position Number（位置號）</dt><dd>{position}</dd><dt>Binary Path（位置路徑）</dt><dd>{path}</dd><dt>Plan（方案）</dt><dd>{plan}</dd><dt>Global cumulative rank（全球累積階級）</dt><dd>{rank}</dd><dt>Owner（持有狀態）</dt><dd>{owner}</dd><dt>Domain Status</dt><dd><StatusBadge status={q.status??'UNAVAILABLE'}/></dd><dt>Active</dt><dd>{companyHeld?'Always Active (Company Rule)':q.activeFlag?'ACTIVE':'INACTIVE'}</dd><dt>Effective</dt><dd>{dateTime(q.effectiveAt)}</dd></dl>{planSource&&<p className="uc-muted">方案來源：{planSource.status==='AVAILABLE'&&planSource.source==='COMPANY_BOOTSTRAP_PROFILE_BINDING'?'Company Bootstrap Profile Binding':planSource.status==='AVAILABLE'&&planSource.source==='QUALIFICATION_PLAN_HISTORY'?'Qualification Plan History':'權威方案證據未提供'}。{planSource.status==='AVAILABLE'&&planSource.source==='COMPANY_BOOTSTRAP_PROFILE_BINDING'?'公司持有本身不會推定為 LEADER。':''}</p>}</>;
