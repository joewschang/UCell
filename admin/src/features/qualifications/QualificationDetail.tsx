...
export function QualificationDetail({id}:{id:string}){
 ...
 let body;
 if(tab==='Overview')body=<dl className="detail-grid"><dt>Ball Number</dt><dd>{q.ballNo??'尚未完成 Binary 放置'}</dd><dt>會員編號</dt><dd>{q.currentHolder?.memberNo??(q.currentCompanyPrincipalId?'公司主體':'未提供')}</dd><dt>Plan</dt><dd>{q.planLevelCode}</dd><dt>Domain Status</dt><dd>{q.status}</dd><dt>Active Flag</dt><dd>{q.currentCompanyPrincipalId?'Always Active (Company Rule)':q.activeFlag?'ACTIVE':'INACTIVE'}</dd><dt>Effective</dt><dd>{dateTime(q.effectiveAt)}</dd></dl>;
