...
export function QualificationsPage(){
 ...
 <div className="toolbar"><input value={search} onChange={e=>setSearch(e.target.value)} aria-label="Qualification ID／持有人姓名／手機／Email" placeholder="Qualification ID／持有人姓名／手機／Email"/><select aria-label="Qualification 狀態" value={status} onChange={e=>setStatus(e.target.value)}><option value="">全部狀態</option><option>EFFECTIVE</option><option>CLOSED</option><option>SUSPENDED</option></select></div>
 <ErrorBox error={list.error}/><div className="uc-master-detail"><Card title={`Qualifications (${rows.length})`}>{rows.map(x=><button className={`list-row ${selected===x.qualificationId?'selected':''}`} key={x.qualificationId} onClick={()=>setSelected(x.qualificationId)}><strong>Q#{qNo(x.qualificationNo)} · {holderName(x)}</strong><span>{x.planLevelCode} · <Badge tone={x.currentCompanyPrincipalId||x.activeFlag?'ok':'neutral'}>{x.currentCompanyPrincipalId?'Always Active (Company Rule)':x.activeFlag?'ACTIVE':'NOT ACTIVE'}</Badge></span><small>{x.qualificationId}</small></button>)}</Card>
