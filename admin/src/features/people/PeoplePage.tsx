...
export function PeoplePage(){
 ...
 const [retailBallNo,setRetailBallNo]=useState(''); const [retailEffectiveFrom,setRetailEffectiveFrom]=useState(''); const [retailReason,setRetailReason]=useState('');
 ...
 const retail=useQuery({queryKey:['person-retail-attribution',selected?.personId],queryFn:()=>get<any>('/admin/retail-referrers/'+selected!.personId),enabled:!!selected});
 const retailCorrection=useMutation({mutationFn:()=>command<any>('/admin/retail-referrers/corrections',{personId:selected!.personId,ballNo:retailBallNo,effectiveFrom:retailEffectiveFrom,reason:retailReason}),onSuccess:()=>void qc.invalidateQueries({queryKey:['person-retail-attribution',selected?.personId]})});
 ...
 return <>
  ...
  <DetailDrawer open={!!selected} title={selected?'會員 360 · '+memberLabel(selected):'會員 360'} onClose={closeDrawer}>
   {selected&&<section aria-live="polite">
    ...
    <section aria-labelledby="person-retail-attribution"><h3 id="person-retail-attribution">商品推薦歸因</h3>{retail.isPending?<LoadingState label="正在讀取商品推薦歸因…"/>:retail.error?<ErrorState message={String(retail.error)} retry={()=>void retail.refetch()}/>:<>{retail.data?.data?.length?<ul>{retail.data.data.map((row:any,index:number)=><li key={index}>{row.ballNo} · {row.source} · {row.effectiveFrom}{row.effectiveTo?` 至 ${row.effectiveTo}`:'（目前）'}</li>)}</ul>:<p className="uc-muted">尚無商品推薦歸因。</p>}<details><summary>前向修正</summary><p className="uc-muted">只接受未來時間；不改寫歷史訂單、快照、Award 或結算。</p><Field label="新推薦 Ball"><input value={retailBallNo} onChange={e=>setRetailBallNo(e.target.value.toUpperCase())}/></Field><Field label="生效時間"><input type="datetime-local" value={retailEffectiveFrom} onChange={e=>setRetailEffectiveFrom(e.target.value)}/></Field><Field label="原因"><input value={retailReason} onChange={e=>setRetailReason(e.target.value)}/></Field><button disabled={retailCorrection.isPending||!retailBallNo||!retailEffectiveFrom||retailReason.length<3} onClick={()=>retailCorrection.mutate()}>送出前向修正</button><ErrorBox error={retailCorrection.error}/></details></>}</section>    <section aria-labelledby="person-account-security"><h3 id="person-account-security">帳號安全</h3>
     {security.isPending?<LoadingState label="正在讀取帳號安全狀態…"/>:security.error?<ErrorState message={String(security.error)} retry={()=>void security.refetch()}/>:<>{(()=>{const value=security.data?.data;const links=value?.identityLinks??[];const masked=(subject:string)=>subject.length<7?'已綁定 LINE':subject.slice(0,3)+'•••'+subject.slice(-3);return <>
      ...
      <h4>LINE 通知交付</h4>{value?.deliveries?.length?<ul>{value.deliveries.map((delivery:any,index:number)=><li key={index}>{delivery.createdAt} · {delivery.notificationType} · {delivery.status}</li>)}</ul>:<p className="uc-muted">尚無通知交付紀錄。</p>}
