...
export function SubscriptionsPage(){
 const [status,setStatus]=useState(''),[ballNo,setBallNo]=useState(''),[selected,setSelected]=useState('');
 const plans=useQuery({queryKey:['subscription-plans'],queryFn:()=>get<any>('/admin/subscriptions/plans')});
 const queue=useQuery({queryKey:['subscriptions',status,ballNo],queryFn:()=>get<any>('/admin/subscriptions'+qs({status,ballNo,take:100}))});
 ...
 <Card title="訂閱清單"><div className="toolbar"><Field label="狀態"><select value={status} onChange={e=>{setStatus(e.target.value);setSelected('')}}><option value="">全部狀態</option>{['PENDING','ACTIVE','SUSPENDED','CANCELLED','COMPLETED'].map(x=><option key={x}>{x}</option>)}</select></Field><Field label="Ball Number"><input value={ballNo} placeholder="例如 TREE-A000001" onChange={e=>{setBallNo(e.target.value.trim().toUpperCase());setSelected('')}}/></Field></div><QueryFeedback query={queue} empty={!rows.length}/>{rows.map((s:any)=><button key={s.subscriptionId} className="list-row" onClick={()=>setSelected(s.subscriptionId)}><strong>{s.plan.planCode} · {s.status}</strong><span>{s.qualification?.ballNo??'Ball evidence 未提供'} · {s.qualification?.currentHolder?.memberNo??'會員編號未提供'}</span><small>{s.startMonth} → {s.endMonth}</small></button>)}</Card>
 {selected&&<Card title="訂閱詳情"><QueryFeedback query={detail}/>{d&&<><p>Ball Number：{d.qualification?.ballNo??'Ball evidence 未提供'}</p><p>會員編號：{d.qualification?.currentHolder?.memberNo??'會員編號未提供'}</p><p>{d.status} · Rule {d.ruleVersionCode} · Snapshot {d.parameterSnapshotHash??'歷史設定 evidence 未提供'}</p><div className="table-wrap"><AdminTable><thead><tr><th>期數</th><th>認列月</th><th>到期時間</th><th>狀態</th><th>認列金額</th><th>RPV</th></tr></thead><tbody>{(d.schedules??[]).map((s:any)=><tr key={s.recognitionId}><td>{s.installmentNo}</td><td>{s.recognitionMonth}</td><td>{dateTime(s.dueAt)}</td><td>{s.status}</td><td>{money(s.recognizedAmount)}</td><td>{s.rpvAmount}</td></tr>)}</tbody></AdminTable></div></>}</Card>}</>;
}
