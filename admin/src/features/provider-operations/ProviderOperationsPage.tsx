...
type ReconciliationStatus='DISCREPANCY'|'FAILED';
type ReconciliationHealth={generatedAt:string;state:'HEALTHY'|'DEGRADED'|'CRITICAL';total:number;exceptions:number;oldestExceptionAt:string|null;counts:{byDomain:Partial<Record<Domain,number>>;byStatus:Partial<Record<'RUNNING'|'MATCHED'|ReconciliationStatus,number>>}};
type ReconciliationItem={providerReconciliationRunId:string;domain:Domain;provider:string;connectionId:string;runKey:string;periodStart:string;periodEnd:string;status:ReconciliationStatus;providerRecordCount:number;internalRecordCount:number;discrepancyCount:number;verificationConfigVersion:string;startedAt:string;completedAt:string|null;correlationId:string};
type ReconciliationQueue={generatedAt:string;items:ReconciliationItem[];limit:number;truncated:boolean};
...
type LineStatus={messaging:{webhookConfigured:boolean;senderConfigured:boolean;workerEnabled:boolean;lastWebhook:{receivedAt:string;status:string}|null};deliveries:Array<{status:string;count:number}>};
const endpoint='/admin/provider-operations/webhooks',reconciliationEndpoint='/admin/provider-operations/reconciliation',domains:Domain[]=['PAYMENT','INVOICE','LOGISTICS'],statuses:Status[]=['RECEIVED','VERIFIED','REJECTED','PROCESSING','PROCESSED','RETRY_PENDING','MANUAL_REVIEW'];
...
const reconciliationColumns:GridColumn<ReconciliationItem>[]=[
 {key:'startedAt',label:'開始時間（台北）',value:r=>r.startedAt,render:r=>time(r.startedAt)},
 ...
 {key:'status',label:'狀態',value:r=>r.status,render:r=><Badge tone="danger">{r.status}</Badge>},
 {key:'period',label:'對帳期間',value:r=>`${r.periodStart} ${r.periodEnd}`,render:r=><>{time(r.periodStart)} ～ {time(r.periodEnd)}</>},
 {key:'counts',label:'Provider／Internal／差異',value:r=>`${r.providerRecordCount}/${r.internalRecordCount}/${r.discrepancyCount}`},
 {key:'config',label:'驗證設定版本',value:r=>r.verificationConfigVersion},{key:'correlationId',label:'Correlation ID',value:r=>r.correlationId},
];
...
export function ProviderOperationsPage(){
 ...
 const lineStatus=useQuery({queryKey:['line-integration','status'],queryFn:()=>get<{data:LineStatus}>('/admin/integrations/line/status'),refetchInterval:60_000});
 ...
 const reconciliationHealth=useQuery({queryKey:['provider-reconciliation','health'],queryFn:()=>get<{data:ReconciliationHealth}>(`${reconciliationEndpoint}/health`),refetchInterval:60_000});
 const reconciliationQueue=useQuery({queryKey:['provider-reconciliation','exceptions'],queryFn:()=>get<{data:ReconciliationQueue}>(`${reconciliationEndpoint}/exceptions?take=50`),refetchInterval:60_000});
 ...
 const h=health.data?.data,b=backlog.data?.data,rh=reconciliationHealth.data?.data,rq=reconciliationQueue.data?.data,line=lineStatus.data?.data;
 ...
 return <><PageHeader title="Provider Webhook 營運" subtitle="監看付款、發票與物流 provider webhook inbox；資料每分鐘自動更新。" actions={<button onClick={()=>{void health.refetch();void backlog.refetch();void reconciliationHealth.refetch();void reconciliationQueue.refetch()}}>重新整理</button>}/>
 <QueryFeedback query={lineStatus}/>{line?.messaging&&<><Card title="LINE Integration"><div className="status-strip"><Badge tone={line.messaging.webhookConfigured&&line.messaging.senderConfigured&&line.messaging.workerEnabled?'ok':'warn'}>{line.messaging.webhookConfigured&&line.messaging.senderConfigured&&line.messaging.workerEnabled?'READY':'CONFIGURATION_PENDING'}</Badge><span className="muted">Webhook {line.messaging.webhookConfigured?'已設定':'未設定'} · Sender {line.messaging.senderConfigured?'已設定':'未設定'} · Worker {line.messaging.workerEnabled?'已啟用':'未啟用'}</span></div><p className="muted">最近 webhook：{line.messaging.lastWebhook?time(line.messaging.lastWebhook.receivedAt)+' · '+line.messaging.lastWebhook.status:'尚無'}。不顯示 LINE secret、token、會員身分或原始事件。</p></Card><div className="metrics">{line.deliveries.map(row=><Metric key={row.status} label={'LINE '+row.status} value={row.count}/>)}</div></>} <QueryFeedback query={health}/>{h&&<><Card title="Inbox 健康狀態"><div className="status-strip"><Badge tone={h.state==='HEALTHY'?'ok':h.state==='DEGRADED'?'warn':'danger'}>{h.state}</Badge><span className="muted">產生時間：{time(h.generatedAt)}（台北時間）</span></div></Card><div className="metrics"><Metric label="Inbox 總數" value={h.total}/><Metric label="到期 backlog" value={h.dueBacklog}/><Metric label="逾期 lease" value={h.expiredLeases}/><Metric label="人工檢視" value={h.manualReview}/></div><div className="grid two"><Card title="Domain 分布"><p>{domains.map(d=>`${d} ${h.counts.byDomain[d]??0}`).join(' · ')}</p></Card><Card title="最舊待處理項目"><p>{time(h.oldestDueReceivedAt)}</p></Card></div></>}
 <QueryFeedback query={reconciliationHealth}/>{rh&&<><Card title="Provider 對帳健康狀態"><div className="status-strip"><Badge tone={rh.state==='HEALTHY'?'ok':rh.state==='DEGRADED'?'warn':'danger'}>{rh.state}</Badge><span className="muted">權威資料更新：{time(rh.generatedAt)}</span></div></Card><div className="metrics"><Metric label="對帳執行總數" value={rh.total}/><Metric label="例外總數" value={rh.exceptions}/><Metric label="差異" value={rh.counts.byStatus.DISCREPANCY??0}/><Metric label="失敗" value={rh.counts.byStatus.FAILED??0}/></div></>}
 <QueryFeedback query={reconciliationQueue} empty={!!rq&&!rq.items.length}/>{rq&&rq.items.length>0&&<Card title="對帳例外"><AdminDataGrid rows={rq.items} columns={reconciliationColumns} rowId={r=>r.providerReconciliationRunId} label="Provider reconciliation exceptions" searchable/>{rq.truncated&&<p className="callout warning">僅顯示前 {rq.limit} 筆，請由 API 篩選縮小範圍。</p>}<p className="muted">僅顯示權威執行狀態與筆數；不在前端解讀金額差異。</p></Card>} <Card title="Backlog 篩選"><form onSubmit={e=>{e.preventDefault();setFilters({...draft,provider:draft.provider.trim()})}}><div className="filter-grid"><Field label="Domain"><select value={draft.domain} onChange={e=>setDraft(v=>({...v,domain:e.target.value as Filters['domain']}))}><option value="">全部</option>{domains.map(v=><option key={v}>{v}</option>)}</select></Field><Field label="Provider"><input maxLength={100} value={draft.provider} onChange={e=>setDraft(v=>({...v,provider:e.target.value}))}/></Field><Field label="狀態"><select value={draft.status} onChange={e=>setDraft(v=>({...v,status:e.target.value as Filters['status']}))}><option value="">預設營運 backlog</option>{statuses.map(v=><option key={v}>{v}</option>)}</select></Field><Field label="載入筆數"><select value={draft.take} onChange={e=>setDraft(v=>({...v,take:Number(e.target.value)}))}>{[25,50,100,200].map(v=><option key={v}>{v}</option>)}</select></Field></div><div className="button-row"><button className="primary" type="submit">套用篩選</button><button type="button" onClick={()=>{setDraft(initial);setFilters(initial)}}>清除</button></div></form></Card>
