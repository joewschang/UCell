...
export function SystemPage(){
 ...
 <Card title="仍需完成"><div className="table-wrap"><table><thead><tr><th>項目</th><th>時點</th></tr></thead><tbody>{gaps.map(g=><tr key={g[0]}><td>{g[0]}</td><td>{g[1]}</td></tr>)}</tbody></table></div></Card>
 <Card title="Backend Routes"><div className="table-wrap"><table><thead><tr><th>Method</th><th>Path</th><th>用途</th></tr></thead><tbody>{backendRoutes.map(r=><tr key={r.method+r.path}><td>{r.method}</td><td className="mono">{r.path}</td><td>{r.label}</td></tr>)}</tbody></table></div></Card>
 </>;
}
