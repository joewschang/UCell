...
export function BinaryTreesPage(){
 ...
 async function load(context=time,after?:string){
  ...
  try{if(id){const response=await get<{data:Detail}>(`/admin/organization/trees/${id}`+qs(context));if(view.current!==target||serial!==requestSerial.current)return;const data=response.data,result=data.result?{...data.result,positions:data.result.positions.map(p=>({...p,qualificationId:p.ballNo??null}))}:null;appliedTime.current=context;setDetail({...data,result});setName(result?.treeName??'');}
 ...
 async function preview(){
  ...
  try{const response=await get<{data:{preflightToken:string;actualSponsorSequenceNo:number;sponsorQualificationId:string}}>(`/admin/organization/trees/${id}/placement-preview`+qs({qualificationMemberNo:qualification,binaryParentBallNo:parent,side,expectedVersion:detail?.result?.topologyVersion}));if(view.current===target&&serial===requestSerial.current)setPreflight(response.data);}
  catch(e){if(view.current===target&&serial===requestSerial.current)setError(e)}finally{if(view.current===target&&serial===requestSerial.current)setBusy(false)}
 }
 ...
 {tree&&<Card title="完整 Binary 樹節點"><p>目前資料截點：{appliedTime.current.asOf}；期間 {appliedTime.current.periodStart} 至 {appliedTime.current.periodEnd}。</p><button disabled={busy} onClick={()=>void loadNodes()}>載入節點</button>{nodes&&<><p>{nodes.parentQualificationId?'直接子節點數':'總節點數'}：{nodes.total??'不可用'}；本頁顯示 {nodes.items.length} 個。統計仍以整棵樹計算。</p>{nodes.status==='UNAVAILABLE'?<p>此時間的節點證據不可用。</p>:<div className="table-wrap"><table><thead><tr><th>深度</th><th>球編號</th><th>位置／路徑</th><th>左右側</th><th>持有類型</th><th>展開</th></tr></thead><tbody>{nodes.items.map(node=><tr key={node.qualificationId}><td>{node.depth}</td><td>{node.ballNo??'證據未提供'}</td><td>{node.binaryPositionNo??'—'} {node.path??''}</td><td>{node.side??'—'}</td><td>{node.ownerType}{node.activeLabel&&<div>{node.activeLabel}</div>}</td><td><button disabled={busy} onClick={()=>void loadNodes(undefined,node.qualificationId)}>展開子節點</button></td></tr>)}</tbody></table></div>}{nodes.nextCursor&&<button disabled={busy} onClick={()=>void loadNodes(nodes.nextCursor!)}>下一頁節點</button>}</>}</Card>}
 {manage&&(!id||tree&&tree.status!=='ARCHIVED')&&<Card title={id?'樹設定':'建立樹'}><div className="form"><Field label="樹名稱"><input maxLength={120} value={name} onChange={e=>setName(e.target.value)}/></Field><Field label="操作原因"><input maxLength={500} value={reason} onChange={e=>setReason(e.target.value)}/></Field><button className="primary" disabled={busy||!name.trim()||!reason.trim()} onClick={()=>void write(id?base+'/settings':base,{treeName:name,reason,...(tree?{expectedVersion:tree.topologyVersion}:{})})}>{id?'儲存名稱':'建立草稿樹（3 顆公司球、7 個位置）'}</button>{tree&&({DRAFT:['ACTIVE','ARCHIVED'],ACTIVE:['CLOSED_TO_NEW'],CLOSED_TO_NEW:['ACTIVE','ARCHIVED']}[tree.status]??[]).map(status=><button key={status} disabled={busy||!reason.trim()} onClick={()=>void write(base+'/settings',{status,reason,expectedVersion:tree.topologyVersion})}>切換至 {status}</button>)}</div></Card>}
  {tree?.status==='ACTIVE'&&(confirmSponsor||place)&&<Card title="Sponsor 確認與放置"><div className="form"><Field label="待放置會員編號"><input inputMode="numeric" maxLength={10} placeholder="例如 2609000001" value={qualification} onChange={e=>{setQualification(e.target.value);setPreflight(null)}}/></Field>{!manage&&<Field label="操作原因"><input value={reason} onChange={e=>setReason(e.target.value)}/></Field>}{confirmSponsor&&<button disabled={busy||!/^\d{10}$/.test(qualification)||!reason.trim()} onClick={()=>void write(base+'/company-sponsor-confirmations',{qualificationMemberNo:qualification,reason})}>確認公司 Sponsor</button>}{place&&<><Field label="父球 Ball Number"><input value={parent} placeholder="例如 TREE-A000001" onChange={e=>{setParent(e.target.value);setPreflight(null)}}/></Field><Field label="位置"><select value={side} onChange={e=>{setSide(e.target.value);setPreflight(null)}}><option value="LEFT">左側</option><option value="RIGHT">右側</option></select></Field><button disabled={busy||!/^\d{10}$/.test(qualification)||!parent} onClick={()=>void preview()}>預檢選定位置</button>{preflight&&<p>Sponsor：{preflight.sponsorQualificationId}；實際序號：{preflight.actualSponsorSequenceNo}。預檢不保留位置。</p>}<button disabled={busy||!preflight||!/^\d{10}$/.test(qualification)||!parent||!reason.trim()} onClick={()=>void write(base+'/placements',{qualificationMemberNo:qualification,binaryParentBallNo:parent,side,expectedVersion:tree.topologyVersion,preflightToken:preflight?.preflightToken,reason})}>放置於選定位置</button></>}<p>第 1 與第 3 位實際直推須位於 Sponsor 左子樹。#4–#7 的位置編號不代表推薦序號。</p></div></Card>}
 </>;
}
