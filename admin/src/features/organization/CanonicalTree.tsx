...
export type CanonicalPosition={positionNo:number;qualificationId:string|null;ownerType:string|null;activeLabel:string|null;parentPositionNo:number|null;side:string|null};
  ...
  <div style={{display:'grid',gridTemplateColumns:'repeat(8,minmax(72px,1fr))',gap:12,minWidth:640,padding:12}}>
   {positions.map(p=>{const company=p.positionNo<=3,empty=!p.qualificationId;
    ...
    return <button key={p.positionNo} type="button" onClick={()=>onSelect(p)}
     ...
     <small style={{display:'block',overflowWrap:'anywhere'}}>{p.qualificationId??'等待首次放置'}</small>
