...
| RPO_RTO | PASS (Stage capability) | Business-approved RPO 1H/RTO 4H assessment is PASS using selected PITR point and measured isolated restore verification; see [G8_RPO_RTO_ASSESSMENT_20260926.md](G8_RPO_RTO_ASSESSMENT_20260926.md). |
...
| SECURITY_EVENT_AUDIT | PARTIAL | Stage Admin `LOGIN_FAILED` audit exercise PASS. Member BOLA implementation is covered by local/isolated evidence, but the browser automation session cannot access Stage session storage and no token may be read/exported; authenticated Stage BOLA exercise remains pending controlled human UAT. Role/grant change evidence also requires a named operator and controlled change. |
