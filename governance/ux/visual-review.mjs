...
const directory=new URL('./references/',import.meta.url);mkdirSync(directory,{recursive:true});
...
try{
 ...
 writeFileSync(new URL('results.json',directory),JSON.stringify({sourceCommit:sha,results,modalFocusTrap:'PASS',qualificationFeedback:'PASS',mobileNavTouchTargets:'PASS',formalLiff:'NOT_VERIFIED',formalEntra:'NOT_VERIFIED'},null,2)+'\n');console.log(`UX_VISUAL_RESPONSIVE_PASS: ${results.length} references; modal keyboard containment/Escape; qualification feedback; mobile navigation touch targets. Member visual MOCK ONLY.`);
