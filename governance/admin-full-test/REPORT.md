...
補充：actor修正後，IdempotencyService對既有四種create操作lookup舊 :system scope的已提交回應；相同key/payload回原結果，不同payload拒絕409，未提交舊回應拒絕409，不另建立資料。新增真正舊呼叫方式的Person回歸（service以無actor執行原namespace，再由目前API重送）並確認只有1個Person。原response/ledger不修改；本轮没有执行official ledger write或银行动作。相关BackendBuild/rootTests/Security/TODO再次重跑，完整矩阵保留FAIL blocker。
最終結果：43項HTTPrequests PASS（新增舊scope重送及payload衝突），API恢復後重新全跑PASS。Browser實際Person留空欄位提交成功；DRAFT Submit無body成功變SUBMITTED；舊草稿Approve遇Binary slot已佔用的正常拒絕，保留規則不修成強行通過。兼容性測試的初始runner路徑及plain-object/實際DTO mismatch另列Test implementation，已改用原CreatePersonDto，未修改requestHash或historical hash。部分早期JSON沒有總result欄位，不應由空results宣稱PASS；最後runner明列PASS/FAIL與failure。最終checkpoint586bf3d（actor與gates後、legacy兼容前）。
